...
import {Comment, Project} from "@/utils/types";
import {Button} from "@/components/ui/button";
import ProjectTable from "@/components/ProjectTable";
import TransactionTable from "@/components/TransactionTable";

const ProjectDetails = ({ params }: { params: { projectId: number } }) => {
    ...
    const fetchProjectDetails = async (projectId: number) => {
        try {
            const response = await fetch(`http://localhost:5034/api/projects/${projectId}`);
            if (response.ok) {
        ...
        <div className="p-4">
            <div className="mt-4 flex flex-row space-x-4 h-[300px]">
                <div className="bg-white border border-gray rounded-md p-4 w-1/3 h-full">
                    <div className="mb-4 font-semibold text-[20px]">Project Details</div>
            ...
            <div className="bg-white border border-gray rounded-md mt-8">
                <TransactionTable params={{projectId: projectId}}/>
            </div>
        </div>
    );
};
